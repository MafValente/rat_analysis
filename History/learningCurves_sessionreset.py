...
def compute_view_curve(name, filter_fn, df):
    ...
    # ------------------------------------------------------------
    # Smooth curves using session-aware EWMA (reset each session)
    # ------------------------------------------------------------
    df_view["smooth_completion"] = (
        df_view.groupby([SUBJECT_COL, SESSION_COL])["is_completed"]
            .apply(lambda x: x.ewm(span=SPAN, adjust=False).mean())
            .reset_index(level=[0,1], drop=True)
    )

    df_view["smooth_accuracy"] = (
        df_view.groupby([SUBJECT_COL, SESSION_COL])["is_correct"]
            .apply(lambda x: x.ewm(span=SPAN, adjust=False).mean())
            .reset_index(level=[0,1], drop=True)
    )
